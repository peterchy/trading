...
### 🔒 Skill Installation — MUST Vet First!

**Before installing ANY new skill**, run the `skill-vetter` security check:
1. `read workspace/skills/skill-vetter/SKILL.md` to follow the vetting protocol
2. Source Check — 来源、作者、下载量、最后更新
3. Code Review — 逐文件检查红标（curl到未知URL、读取敏感文件、base64、eval等）
4. Permission Scope — 评估文件/网络/命令权限
5. Output Vetting Report 并分类风险等级

🚨 未经过 vetting 的 skill 不得安装。
🔴 HIGH 及以上风险需飞浪人工确认。
...
###「持仓分析」
当客户说「持仓分析」时：
1. 先请客户提供持仓明细（股票代码+股数+成本）
2. 重新获取所有持仓的最新行情数据
3. 逐一分析每只股票（基本面→技术面→量化行为金融→综合建议）
4. 输出完整持仓分析报告

###「深度分析」
当客户说「深度分析XX股票」时：
1. 加载 stock-analyst 框架
2. 获取该股票行情+财务+新闻数据
3. 四步分析（基本面→技术面→情绪→综合建议）
4. 输出深度分析报告

###「建仓选股」
当客户说「建仓选股」时，按以下流程执行：
1. **板块筛选**：找出今日热门板块，判断持续性
2. **个股筛选**：量比>1.5, 涨幅3-5%, 市值50-300亿
3. **资金流验证**：主力净流入>0，北向资金偏好
4. **决策**：结合分散度和资金强度排序输出
...
**第一步：板块筛选**
- 先看热门板块，重点关注新增的热门板块
- 判断驱动因素：事件性炒作（一日游）还是基本面/行业性影响（有持续性）
- 只选可能有持续影响的行业
